library(nimble)
library(coda)
source("sim.USCR.Dcov.multisession.R")
source("sim.USCR.Dcov.R")
source("NimbleModel USCR Poisson Dcov Marginal Multisession.R")
source("NimbleFunctions USCR Poisson Dcov Marginal Multisession.R")
source("init.USCR.Dcov.multisession.R")
...
##Fit model in Nimble##
M <- rep(250,N.session) #data augmentation level
...
nimbuild <- init.USCR.Dcov.multisession(data,inits,M=M)
...
Nimdata <- list(y.noID=nimbuild$y.noID, #no ID detections
...
Rmodel <- nimbleModel(code=NimModel, constants=constants, data=Nimdata,check=FALSE,
                      inits=Niminits)
config.nodes <- c('lam0.fixed','sigma.fixed','D0',"D.beta1")
...
for(g in 1:N.session){
  ...
  conf$addSampler(target = c("N"),
                  type = 'zSampler',control = list(g=g,J=J[g],M=M[g],z.ups=z.ups[g],
...
#add block sampler for lam0.fixed, sigma.fixed, and/or D0 if posteriors correlated (usually are)
conf$addSampler(target = c("lam0.fixed","sigma.fixed","D0"),type = 'RW_block',
